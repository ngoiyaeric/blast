...
class Scheduler:
    ...
    async def stream_task_events(self, task_id: str) -> AsyncIterator[Union[AgentReasoning, AgentHistoryListResponse]]:
        ...
        uncompleted_deps = [dep_id for dep_id in dependency_ids if not self.tasks[dep_id].is_completed]
                    ...
                    if current_task.executor_run_task and current_task.executor_run_task.done():
                        try:
                            ...
                            if result and current_task_id not in yielded_completed_tasks:
                                ...
                                yield AgentHistoryListResponse.from_history(history=result, task_id=current_task_id)
                        except Exception as e:
                            logger.error(f"Task {current_task_id} failed: {e}", exc_info=True)

                # Handle newly completed tasks
                elif (
                    current_task.is_completed and current_task.result and current_task_id not in yielded_completed_tasks
                ):
                    yielded_completed_tasks.add(current_task_id)
                    yield AgentHistoryListResponse.from_history(history=current_task.result, task_id=current_task_id)
    ...
    async def start_task_exec(self, task_id: str, executor: Executor, cached_plan: Optional[AgentHistoryList] = None):
        ...
        if cached_plan:
            ...
            plan = await self.planner.plan(
                task.description, subtask_depth=depth, previous_tasks=previous_tasks, initial_url=task.initial_url
            )
        ...
        if result:
            ...
            if task.success:
                self.cache_manager.update_result(
                    task_lineage=self.get_lineage(task_id), result=result, cache_control=task.cache_options
                )

                self.cache_manager.update_plan(
                    task_lineage=self.get_lineage(task_id), plan=result, cache_control=task.cache_options
                )
        else:
