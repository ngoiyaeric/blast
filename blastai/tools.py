...
class Tools:
    ...
    def _register_subtask_tools(self, scheduler: Scheduler):
        ...
        async def launch_subtask(
            ...
            if num_copies == 1:
                return ActionResult(extracted_content=f'🚀 Launched subtask {task_ids[0]} to "{task}"')
            else:
        ...
        async def get_first_subtask_result(comma_separated_list_of_task_ids: str) -> ActionResult:
            ...
            task_ids = [
                tid.strip() for tid in comma_separated_list_of_task_ids.split(",") if tid.strip() != self.task_id
            ]
            if not task_ids:
...
Explain the content of the chunk and that the requested information is not available in the chunk. Respond in JSON format.\nQuery: {query}\n Website chunk:\n{chunk}"""
                ...
                return ActionResult(success=False, error=f"Failed to extract structured content: {str(e)}")
    ...
    def _register_human_tools(self):
        ...
        async def ask_human(
            prompt: str, allow_takeover: bool = False, browser_session: Optional[BrowserSession] = None
        ) -> ActionResult:
            ...
            try:
                ...
                request = HumanRequest(
                    task_id=self.task_id, prompt=prompt, allow_takeover=allow_takeover, live_url=live_url
                )
                ...
                if not isinstance(response, HumanResponse) or response.task_id != self.task_id:
                    return ActionResult(success=False, error="Invalid response received from human")
                ...
                return ActionResult(success=False, error=f"Failed to get human assistance: {str(e)}")
