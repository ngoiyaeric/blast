            ...
            try:
                ...
                for task_id in ready_tasks[:]:
                    ...
                    if sorted_tasks and task_priority(sorted_tasks[0]) < 999:
                        ...
                        cached_plan = self.cache_manager.get_plan(task_lineage, task.cache_options)
                ...
                running_executors = sum(
                    1 for task in self.scheduler.tasks.values() if task.executor and not task.is_completed
                )
                completed_with_executor = sum(1 for task in self.scheduler.tasks.values() if task.is_completed)

                if (
                    tasks_allocated > 0
                    or tasks_not_allocated != self._prev_not_allocated
                    or completed_with_executor != (self._prev_completed_with_executor or completed_with_executor)
                ):
                    logger.debug(
        ...
        while self._running:
            try:
                ...
                def get_num_unscheduled_tasks():
                    return sum(
                        1 for task in self.scheduler.tasks.values() if not task.is_completed and not task.executor
                    )
                ...
                if not self.check_constraints_sat():
                    ...
                    running_tasks = [t.id for t in self.scheduler.tasks.values() if not t.is_completed and t.executor]
                    ...
                    for group in reversed(priority_groups):
                        for task_id in group.task_ids:
                            task = self.scheduler.tasks[task_id]
                            if task.executor:
                                running = sum(1 for t in self.scheduler.tasks.values() if t.executor)
